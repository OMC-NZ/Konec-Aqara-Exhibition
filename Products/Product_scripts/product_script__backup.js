...
// Configuration-based gallery switching
(function(){
  const configOptions = Array.from(document.querySelectorAll('.config-option'));
  const galleryContainer = document.querySelector('.gallery-container');
  if (!galleryContainer || !configOptions.length) return;

  // normalise dataset key (some images use data-config, one had data-colour)
  const imgs = Array.from(galleryContainer.querySelectorAll('img')).map(img => {
    const cfg = img.dataset.config || img.dataset.colour || '';
    return { el: img, config: cfg };
  });

  let currentConfig = configOptions[0].dataset.config || configOptions[0].dataset.config;
  let currentIndex = 0;

  function visibleImagesFor(cfg) {
    return imgs.filter(i => i.config === cfg).map(i => i.el);
  }

  function showConfig(cfg) {
    currentConfig = cfg;
    configOptions.forEach(o => o.classList.toggle('active', o.dataset.config === cfg));

    const visible = visibleImagesFor(cfg);
    // hide all then show matching ones
    imgs.forEach(i => i.el.style.display = (i.config === cfg) ? '' : 'none');

    // set active image to first visible
  ...
  // expose changeImage for existing markup which calls changeImage(n)
  window.changeImage = function(n){
    const visible = visibleImagesFor(currentConfig);
    if (!visible.length) return;
    currentIndex = (currentIndex + n + visible.length) % visible.length;
    visible.forEach((img, idx) => img.classList.toggle('active', idx === currentIndex));
  };

  // click handlers
  configOptions.forEach(opt => {
    opt.addEventListener('click', () => showConfig(opt.dataset.config));
  });
